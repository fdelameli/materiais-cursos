...
public class ManterCliente extends JInternalFrame {
			...
			jButtonExcluir.addActionListener( new ActionListener() {
				public void actionPerformed(ActionEvent arg0) {
					if (jTextFieldCidade.getText().length() <= 0) {
						JOptionPane.showMessageDialog(null, "Selecione um cliente antes de tentar excluir",
	...
	private void salvar() {
		...
		if (jTextFieldCodigo.getText() != null && jTextFieldCodigo.getText().length() > 0){
			...
			try {
				...
				JOptionPane.showMessageDialog(null, "Cliente " + cliente.getNome() + " Atualizado com Sucesso!", "Mensagem", JOptionPane.INFORMATION_MESSAGE);
				limpaTela();
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Cliente " + cliente.getNome() + " Inserido com Sucesso!", "Mensagem", JOptionPane.INFORMATION_MESSAGE);
			limpaTela();
	...
	private void excluir () {
		...
		try {
			...
			List<ReservaDTO> lista = rm.lisCodigoCliente();
			
			boolean encontrou = false;
			
			for (int i = 0; i < lista.size(); i++) {
				if (cliente.getCodigo() == lista.get(i).getCliente()) {
					encontrou = true;
				}
			}
			...
			if (encontrou == true) {
				JOptionPane.showMessageDialog(null, "Este Cliente possui uma reserva\n" +
						"Você não pode excluí-lo", "Erro", JOptionPane.ERROR_MESSAGE);
				
			} else {
				clienteModel.delete(cliente);
				JOptionPane.showMessageDialog(null, "Cliente " + cliente.getNome() +" Excluido com Sucesso!", "Mensagem", JOptionPane.INFORMATION_MESSAGE);
				limpaTela();
			}
