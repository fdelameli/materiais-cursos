...
public class ManterReserva extends JInternalFrame {
	...
	private JComboBox getJComboBoxCliente() {
		if (jComboBoxCliente == null) {
			...
			for (int i = 0; i < tamanho; i++) {
				jComboBoxCliente.addItem(listaClientes.get(i).getNome());
			}
	...
	private JComboBox getJComboBoxQuarto() {
		if (jComboBoxQuarto == null) {
			...
			for (int i = 0; i < tamanho; i++) {
				jComboBoxQuarto.addItem(listaQuartos.get(i).getNumero());
			}
	...
	public void buscar(){
		...
		String nomeCliente = listaClientes.get(index).getNome();
		JOptionPane.showMessageDialog(null, nomeCliente);
		BuscarReserva br = new BuscarReserva(this, nomeCliente);
