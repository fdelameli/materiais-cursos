 ...
 * 
 */
public class Analisador implements ConstantsInterface {
	...
	/**
	 * Objeto do tipo Tabela.
	 */
	private TabelaSimbolosReconhecidos tabela;
	...
	public Analisador() {
		...
		this.count = 0;
		this.line = 1;
		this.tabela = new TabelaSimbolosReconhecidos();
	}
	 ...
	 */
	public TabelaSimbolosReconhecidos efetuarAnalise(String prog) {
		this.programa = prog;
		...
		while (this.count < programa.length()) {
			...
			if (isLetter(c)) {
				...
				if (RESERVED_WORDS_TABLE.contains(token.toUpperCase())) {
					tabela.setaValoresNaTabela((SYMBOLS_TABLE.indexOf(token.toUpperCase())+1), token, PALAVRA_RESERVADA);
				
				} else if (OPERATORS_TABLE.contains(token.toUpperCase())) {
					tabela.setaValoresNaTabela((SYMBOLS_TABLE.indexOf(token.toUpperCase())+1), token, OPERADOR);

				} else {
					// Verifica se o identificador tem mais de 30 caracteres.
					if (token.length() > 30) { 
						tabela.setaValoresNaTabela((SYMBOLS_TABLE.indexOf(IDENTIFICADOR)+1), token.substring(0,30), IDENTIFICADOR);	
						log += "Identificador com " + token.length() + " caracteres encontrado na linha " + line + ".\n";
					} else {
						tabela.setaValoresNaTabela((SYMBOLS_TABLE.indexOf(IDENTIFICADOR)+1), token, IDENTIFICADOR);
					}
				}
				setVazio();
			}
			...
			// Identifica os números inteiros.
				...
				tabela.setaValoresNaTabela((SYMBOLS_TABLE.indexOf(INTEIRO)+1), token, INTEIRO);
				setVazio();
			}
			...
			// Verifica se existem dois caracteres aspas (") para que seja um literal.
				...
				tabela.setaValoresNaTabela(SYMBOLS_TABLE.indexOf(LITERAL)+1, token, LITERAL);
				setVazio();
			}
				...
				do {
					...
					try {
						if (isDot(""+programa.charAt(count+1)) && isDot(temp) && (count+1 != programa.length())) {
							++count;
							token += c;
							tabela.setaValoresNaTabela(SYMBOLS_TABLE.indexOf(token)+1, token, SIMBOLO_ESPECIAL);
							if (isEndOfFile()) {break;}
							c = getNextChar();
							setVazio();
						} 
							...
							++count;
							token += "=";
							tabela.setaValoresNaTabela(SYMBOLS_TABLE.indexOf(token)+1, token, OPERADOR);
							if (isEndOfFile()) {break;}
							c = getNextChar();
							setVazio();	
						}
			...
			// Verifica se é um caracter de final de arquivo.
			else if (c.matches("[$]")) {
				log += "Caracter de final de arquivo encontrado na linha " + line + ".\n";
				if (isEndOfFile()) {break;}
				c = getNextChar();
				setVazio();
			}
