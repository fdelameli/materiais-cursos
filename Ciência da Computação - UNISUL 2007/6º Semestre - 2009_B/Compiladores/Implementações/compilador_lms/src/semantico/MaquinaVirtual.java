...
/**
 * Classe utilizada pela classe "Hipotetica" para armazenar as informações de
 * uma instrução. Esta classe, bem como as classes "AreaInstrucoes",
...
class Hipotetica {
	...
	public static int nv; // número de variáveis;
	public static int np; // número de parâmetros;
	...
	public static void Interpreta(AreaInstrucoes AI, AreaLiterais AL) {
		...
		while (operador != 26) {// Enquanto instrução diferente de Pare
			...
			case 20:// DSVF
				if (S[topo] == 0) {
					p = a;
					// topo=topo-1; //A no material impresso esta linha está
					// fora do "if"!
				}
				topo = topo - 1;
			...
			case 21:// LEIT
				topo = topo + 1;
				leitura = JOptionPane.showInputDialog(null, "Informe o valor:",
						"Leitura", JOptionPane.QUESTION_MESSAGE);
				// System.out.print("Leia: "); A
				(S[topo]) = Integer.parseInt(leitura); // problema aqui A
				break;

			case 22:// IMPR
				JOptionPane.showMessageDialog(null, "" + S[topo], "Informação",
			...
			case 23:// IMPRLIT
				if (a >= AL.LIT) {
					...
					JOptionPane.showMessageDialog(null, "" + AL.AL[a],
