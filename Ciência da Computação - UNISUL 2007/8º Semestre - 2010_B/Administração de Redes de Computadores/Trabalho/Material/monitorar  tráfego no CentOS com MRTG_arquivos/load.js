...
if (this._isNestedData(value)) {
if (!this._isAlreadySerialized(stack, value)) {
stack.push(value);
return escape(prmName) + "=" + this.nestedDataMarker + this._serialize(value, stack);
}
return '';
}
else
return escape(prmName) + "=" + escape(value);
},
...
var prmName = unescape(pairs[i].split('=')[0]);
var prmVal = pairs[i].split('=')[1];
if (prmName && (prmVal || prmVal == '')) {
var subObjMatch = prmVal.match(new RegExp("^" + this.nestedDataMarker + "(.*)$"));
if (subObjMatch) {
res[prmName] = this.unserialize(subObjMatch[1]);
} else {
res[prmName] = unescape(prmVal);
}
}
}
return res;
...
__AtlasRichMedia__.addModule(
{
moduleId : "helperIframeDataSerializer_a2",
serializer: __AtlasRichMedia__.getModule('dataSerializer_a2'),
...
__AtlasRichMedia__.addModule(
{
moduleId : "safeServeIframeDataSerializer_a3",
entry: function(context) {
if(context)
this._context = context;
else
...
dataSerializer: __AtlasRichMedia__.getModule('dataSerializer_a2')
}
}
},
...
{
var serializedTplParams = __AtlasRichMedia__.getModule('helperIframeDataSerializer_a2').serializeByPaid(destObj.paid);
